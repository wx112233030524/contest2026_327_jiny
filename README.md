...
这是本项目最重要的一条设计决策。大模型会算错——这是事实，不值得赌。所以整定链路里加了一道闸门：AI 给出的**电流环 4 个 PI 参数**（`Kp_Id` / `Ki_Id` / `Kp_Iq` / `Ki_Iq`），必须落在**经典控制理论解算出的理论值 ±50% 区间内**才允许下发：
...
超出区间的结果被直接拒绝，返回 `FOC_TUNE_WHITELIST_FAIL`，电机保持原参数。**AI 负责在合理范围内给出好参数，白名单负责保证它永远不会给出致命的电流环参数**——这才是 AI 落到硬件上该有的形态：让模型做它擅长的（在约束内搜索、权衡用途偏好），把安全边界交给确定性代码。

> **速度环 2 个参数（`Kp_Speed` / `Ki_Speed`）目前不在白名单比对范围内**，这是已知
> 限制而非疏漏：它们靠提示词里写死的降阶公式引导（`foc_ai_tuner.c:110`，
> 速度环带宽 = 电流环带宽 / 10）。差别是实质性的——电流环由确定性代码**事后否决**，
> 速度环只是**事前引导**，模型不照做时没有代码拒绝。补齐方式是把速度环改为由
> 已通过校验的电流环参数直接推算，见 `技术报告.md` 8.2 节第 3 条。
...
- ✅ MiMo API 调用成功返回 PI 参数，电流环白名单校验生效
...
- **速度环 2 个参数没有白名单保护**（只有电流环 4 个有），详见上文「核心亮点」下的说明
- 当前白名单是固定 ±50% 区间，尚未按电机用途（云台/航模/机器人）做差异化收紧
